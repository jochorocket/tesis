...
int main(int argc, char *argv[]) {
	...
	//double(*_benchFunc)(vector<double>) = _maytasFunc;
	double(*_benchFunc)(vector<double>) = _holdertableFunc;

	double _centerVal = 0;
	double _pulseWidth = 10;	//	5 when Arckley
								//	10 when Booth
								//	10 when Maytas
								//	10 when Holder table

	//int _classicalIndGen = 3;
	int _classicalIndGen = atoi(argv[1]);
	//int _noPartitions = 4;
	int _noPartitions = atoi(argv[2]);
	int _crossoverOccurence = _noPartitions - 1;
